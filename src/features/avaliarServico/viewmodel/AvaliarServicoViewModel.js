...
export default {
    ...
    methods: {
        ...
        async submitEvaluation() {
            if (this.rating === 0) {
                this.errorMessage = 'Por favor, selecione uma avaliação de 1 a 5 estrelas.';
                return;
            }
            ...
            try {
                ...
                // Para testes, o ID do consumidor está fixo.
                // Descomente o bloco abaixo para usar o usuário autenticado.
                /*
                const usuario = JSON.parse(localStorage.getItem('usuario'));
                if (!usuario || !usuario.id) {
                    throw new Error("Usuário não autenticado.");
                }
                const usuarioConsumidor = usuario.id;
                */
                const usuarioConsumidor = 1; // ID do usuário fixo para teste
                ...
                setTimeout(() => this.$router.push({ name: 'servicoDetalhe', params: { id: servicoId } }), 2000);
            } catch (error) {
